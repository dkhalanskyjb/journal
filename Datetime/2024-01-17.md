...
Right now people are using `monthNumber` (<https://grep.app/search?q=monthNumber&words=true&filter[lang][0]=Kotlin>)
to
...
`DateTimeFormatBuilder.WithDate`: we decided that we should keep `Number` for
consistency with the corresponding field and in order to immediately present
a choice between string-based and number-based representations of the month, and
if we remove `monthNumber`, the first reason is no longer valid.
